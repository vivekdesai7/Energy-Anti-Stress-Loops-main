...
public class BoardHolder : MonoBehaviour, IHolder
{
    ...
    public void hold(Node node)
    {
        ...
        Debug.Log("node.nodeType---" + node.nodeType); 
        if(node.nodeType!=NodeType.boltRight && node.nodeType != NodeType.boltLeft
            && node.nodeType != NodeType.boltDown && node.nodeType != NodeType.boltUp)
        {
        node.transform.eulerAngles = new Vector3(0f,0f,90f);
        }
